...
ERD 1  	- Modularization and Clear Responsibility: Each table focuses on a single business domain (articles, users, countries, etc.), making it easy to maintain.
...
ERD 2  	- Low Data Redundancy: It follows the 3rd Normal Form (3NF). Through the association of primary keys and foreign keys, it avoids the repeated storage of information such as regions and countries.
...
- Flexible Query: It supports cross-table association and can realize multi-dimensional analysis of region-country-resources/information.	- Potential Query Performance Issues: Cross-table associations may slow down when dealing with large amounts of data, such as complex cross-table statistics.
...
ERD 3  	- Clear Business Division: Each table focuses on a single domain (continent, country, article, etc.), making the structure easy to understand and maintain.
...
- Intuitive Relationship Mapping: Entity associations 	- Performance Overhead from Multi-table JOINs: Querying across related tables (e.g., retrieving an article with its country and continent details) requires multiple JOINs, which is inefficient when data volume is large.
